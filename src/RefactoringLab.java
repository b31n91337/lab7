...
class Transcript {
    ...
    public void generateTranscriptForWidth(int width) {
        ...
        printHeaderString(width, transcriptBuilder, studentName);
        printHeaderString(width, transcriptBuilder, studentId);
        printHeaderString(width, transcriptBuilder, semesterName);
        ...
        for (ClassResult result : classResults) {
            ...
            currentLength = appendCompute(width, transcriptBuilder, result.classCode().length(), currentLength);
            ...
            currentLength = appendCompute(width, transcriptBuilder, gradePointsString.length(), currentLength);
            ...
            currentLength = appendCompute(width, transcriptBuilder, creditsString.length(), currentLength);
	...
	private int appendCompute(int width, StringBuilder transcriptBuilder, int stringLength, int currentLength) {
		if (currentLength + 1 + stringLength > width) {
	...
	private void printHeaderString(int width, StringBuilder transcriptBuilder, String string) {
		int spacesBeforeString = (width - string.length()) / 2;
        for (int i = 0; i < spacesBeforeString; ++i) {
            transcriptBuilder.append("-");
        }
        transcriptBuilder.append(string);
        for (int i = 0; i < spacesBeforeString; ++i) {
            transcriptBuilder.append("-");
        }
        transcriptBuilder.append("\n");
